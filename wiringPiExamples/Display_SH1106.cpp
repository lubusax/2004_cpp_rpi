...
void Display_SH1106::entireDisplayOn(void) { 
  int control = 0x00;   // Co = 0, D/C = 0
  int command = SH1106_DISPLAYALLON;
  // int wiringPiI2CWrite (int fd, int data)
  wiringPiI2CWrite (_fileDevice, control);
  wiringPiI2CWrite (_fileDevice, command);
}
    ...
    for( j = 0; j < 8; j++){        
      ...
      for ( k = 0; k < width; k++, p++) {
        result= i2c_smbus_access (
          _fileDevice, I2C_SMBUS_WRITE, buffer[p], I2C_SMBUS_BYTE, NULL);
        // result = wiringPiI2CWrite (_fileDevice, buffer[p]);
        // if (result < 0) {
        //   printf(stderr, "%s\n", explain_ioctl(_fileDevice, request, data));
        //   //exit(EXIT_FAILURE);
        // }
